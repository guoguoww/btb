                        ...
                        <div class="det-right">
                            ...
                            <div class="det-explain" v-if="buyer()">
                                ...
                                <div style="text-align:center;">
                                    <img style="height:100px;" :src="img" alt="" v-preview="img" >
                                </div>                                 
                            </div>
                            ...
                            <template v-if='buyer()'>
                                <div class="det-time" v-if="!timeout||order.status==0">
                                    ...
                                    <div  class=" pz" @change="changImg('qrcodePic', $event)"  v-show="order.status==0">
                                        <span>上传交易凭证图片</span>
...
export default {
    ...
    methods: {
        ...
        //获取订单
        getorder() {
            if (this.order.cancelPayment&&this.order.cancelPayment!=0) {
                return
            }
            this.$axios
                .post(
                ...
                .then(res => {
                    if (!res.data.errorCode) {
                        ...
                        this.settimer();
                        this.$axios.get(`/api/image/${this.order.voucher}`).then(res=>{
                        this.img=res.data.replace(/\s+/g,"")})
                        if (
                            this.order.status == 0 &&
                            this.buyer() &&
                            !this.timeout
        ...
        imgurl(id) {
            ...
            return `http://192.168.0.222:8080/bct_front/api/image/display/${id}`;
        },
        //获取剩余付款时间
        gettime() {
            var t =
        ...
        pay() {
            if (!this.img) {
                this.$Message.warning('未上传付款凭证')
                return
            }
            this.$axios.post(`api/trade/c2c/user_payment`,{ tradeId:this.order.id, voucher:this.img})
                .then(res => {
                    if (!res.data.errorCode) {

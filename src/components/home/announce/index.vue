<template>
    <div class="wrap">
      <div class="container">
        ...
        <div class="dw-body">
          <ul>
            <router-link :to="{path: '/announceDetail',query: {queryId: item.id}}" tag="li" v-for="(item,index) of announceInfo" :key="index"><span>{{item.title}}</span><span>{{item.createTime}}</span></router-link>
          </ul>
          <div class="nodata" v-show="announceOutShow">
            <img src="../../../../static/imgs/record.png">
            <p class="" style="margin-left:-4px;">无公告记录</p>
          </div>
        </div>
      </div>
...
<script>
    export default {
      ...
      methods: {
        announceList() {
          this.$axios.post('/api/user/announce',{start: this.reqParams.start, rows: this.reqParams.rows}).then(res => {
            if(res.data.errorCode == 0) {
              if(res.data.page.data.length != 0) {
                this.announceInfo = res.data.page.data;
                this.announceOutShow = false;
